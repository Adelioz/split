...
func NewRouter(l *zap.Logger, s *service.Service) *mux.Router {
	...
	expenseRouter.Path("/{id}/").Methods(http.MethodGet).Handler(GetExpense(s))       // curl http://localhost:65000/expense/12/ -X GET -v
	expenseRouter.Path("/{id}/").Methods(http.MethodDelete).Handler(DeleteExpense(s)) // curl http://localhost:65000/expense/12/ -X DELETE -v
